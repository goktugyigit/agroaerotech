// Blog listesi getirme (GET) - Public erişim için session kontrolü kaldırıldı
export async function onRequestGet({ env }) {
  try {
    ...
    // Çoklu dil sütunları ile blog listesi getir
    let results = [];
    try {
      const queryResult = await env.BLOG_DB.prepare(
        `SELECT id, 
                ...
                image, read_time, created_at, updated_at
         FROM blogs 
      ...
      console.log('Admin blog listesi başarılı:', results.length, 'blog bulundu');
    } catch (error) {
      console.error('Admin blog listesi hatası:', error);
      results = [];
    }
    
    console.log('Blog listesi başarıyla getirildi:', results.length, 'blog');
...
// Yeni blog ekleme (POST)
export async function onRequestPost({ request, env }) {
  try {
    console.log('Blog ekleme isteği alındı');
    ...
    if (existingSlug) {
      console.error('Slug zaten kullanılıyor:', data.slug);
      return new Response(JSON.stringify({
        success: false,
    ...
    console.log('Blog veritabanına ekleniyor...');
    const result = await env.BLOG_DB.prepare(`
      INSERT INTO blogs (title_tr, title_en, title_ru, title_az, slug, category, 
                        description_tr, description_en, description_ru, description_az,
                        content_tr, content_en, content_ru, content_az, 
                        image, read_time, published)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      data.title_tr,
      data.title_en || null,
      ...
      data.read_time || 5,
      1 // Varsayılan olarak yayınlanmış
    ).run();
    
    console.log('Blog başarıyla eklendi, ID:', result.meta.last_row_id);
    
    // Blog HTML dosyasını oluştur
    try {
      const blogId = result.meta.last_row_id;
      const newBlog = await env.BLOG_DB.prepare(
        "SELECT * FROM blogs WHERE id = ?"
      ).bind(blogId).first();
      
      if (newBlog) {
        const htmlContent = generateBlogHTML(newBlog);
        console.log('Blog HTML içeriği oluşturuldu');
        
        // HTML içeriğini response'a ekle (manuel olarak dosya oluşturmak için)
        return new Response(JSON.stringify({
          success: true,
          message: "Blog başarıyla eklendi",
          id: result.meta.last_row_id,
          html: htmlContent,
          filename: `blog/${newBlog.slug}.html`
        }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      // HTML hatası olsa bile blog eklendi, sadece uyarı ver
    }
    ...
    return new Response(JSON.stringify({
      success: false,
      message: `Blog eklenirken hata oluştu: ${error.message}`
    }), { 
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
...
// Session kontrolü fonksiyonu (mevcut admin session kontrolünüzü buraya ekleyin)
async function checkAdminSession(request, env) {
  // Burada mevcut admin session kontrolünüzü kullanın
  // Örnek: cookie kontrolü, JWT token kontrolü vs.
  try {
    const cookie = request.headers.get('Cookie');
    if (!cookie) return false;
    
    // Mevcut session kontrolünüzü buraya ekleyin
    // Bu kısım sizin mevcut admin authentication sisteminize göre değişecek
    
    return true; // Geçici olarak true döndürüyorum
  } catch (error) {
    return false;
  }
}
...
<head>
  ...
  <title>${blog.title_tr || blog.title || 'Blog'} | AgroAeroTech Blog</title>
  <meta name="description" content="${blog.description_tr || blog.description || 'Blog açıklaması'}">
      ...
      <div class="row">
        <div class="col-lg-8 mx-auto">
          ...
          <h1 class="display-5 fw-bold mb-4" data-aos="fade-up">
            ${blog.title_tr || blog.title || 'Blog Başlığı'}
          </h1>
      ...
      <div class="row">
        <div class="col-lg-8 mx-auto">
          <div class="blog-content" data-aos="fade-up">
            ${blog.image && blog.image.trim() ? `<img src="${blog.image}" alt="${blog.title_tr || blog.title || 'Blog'}" class="img-fluid rounded shadow mb-4">` : ''}
            
            <p class="lead">
              ${blog.description_tr || blog.description || 'Blog açıklaması'}
            </p>

            ${blog.content_tr || blog.content || 'Blog içeriği'}

            <div class="alert alert-success mt-4">
              <h5><i class="fas fa-lightbulb me-2"></i>Sonuç</h5>
              <p class="mb-0">
                Bu blog yazısı AgroAeroTech tarafından hazırlanmıştır. Daha fazla bilgi için bizimle iletişime geçebilirsiniz.
              </p>
            </div>
          </div>
