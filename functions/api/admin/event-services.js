// Admin - Etkinlik Hizmetleri Başvuruları API
export async function onRequestGet(context) {
    ...
    try {
        ...
        console.error('Admin event services error:', error);
...
export async function onRequestPut(context) {
    ...
    try {
        ...
        ).bind(status || 'yeni', notes || '', id, 'etkinlik').run();
        ...
        console.error('Admin event services update error:', error);
        return new Response(JSON.stringify({ success: false, message: 'Bir hata oluştu' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
        });
...
export async function onRequestDelete(context) {
    ...
    try {
        ...
        console.error('Admin event services delete error:', error);
        return new Response(JSON.stringify({ success: false, message: 'Bir hata oluştu' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
        });
